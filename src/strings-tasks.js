...
function sumOfCodes(str) {
  ...
  return [...str].reduce((sum, num) => sum + num.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  return [...str].filter((vowel) => vowels.includes(vowel)).length;
}
...
function isPalindrome(str) {
  const cleanStr = str
    ...
    .replaceAll(' ', '')
    .replaceAll(',', '');
  const rts = [...cleanStr].reverse().join('');
  return rts === cleanStr;
}
/**
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((long, word) => (word.length > long.length ? word : long), '');
}
